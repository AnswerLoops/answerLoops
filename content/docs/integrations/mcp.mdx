...
1. Go to **Settings → API Keys** (workspace owners and admins only — a key grants read access to the whole org's tickets and knowledge base plus metered AI spend, so minting and revoking them is an admin action)
...
- Rate limited per organization (shared across all of that org's keys), enforced globally across every running instance so the limit holds regardless of how many instances are deployed behind it: 60 requests/minute, plus a generous per-IP limit (300/minute) that applies before a key is even resolved. The same counters back the [Agent API](/docs/integrations/agent-api), so a key can't get a second allowance by switching surfaces
