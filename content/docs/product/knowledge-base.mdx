...
Connect a Notion workspace and pull selected pages and databases into the KB — useful for teams that keep their support content in Notion rather than a published docs site.

1. Go to **Knowledge Base → Notion**
2. Authorize AnswerLoops against your workspace
3. Pick the pages and databases to sync
4. Click **Sync now** for an immediate import; re-sync picks up edited pages and skips unchanged ones

Each Notion page becomes its own source entry, split into chunks and embedded the same way as every other source.
