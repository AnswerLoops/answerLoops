...
Telegram doesn't push messages to you automatically — you have to tell it where to send them. After saving your token, a **Register webhook** button appears on the Telegram card. Click it once; AnswerLoops calls Telegram's `setWebhook` API with your app's public URL and a per-org secret, and Telegram starts POSTing new messages to `/api/telegram/webhook`.

<Note>
  You need to re-register the webhook any time your public URL (`AUTH_URL`) changes, or if you rotate the bot token.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Webhook registration fails">
    Registration calls Telegram's `setWebhook` API with your app's public URL. If `AUTH_URL` isn't set to a real, publicly reachable HTTPS URL, Telegram will refuse it — `localhost` and private/internal hostnames don't work here. Set `AUTH_URL` to your public app URL and click **Register webhook** again.
  </Accordion>
