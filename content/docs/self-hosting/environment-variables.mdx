...
No environment variable. Each org connects its own Notion workspace in **Settings → Notion** by pasting an internal integration token, which is stored encrypted — make sure `ENCRYPTION_KEY` (above) is set before connecting in any real deployment.
